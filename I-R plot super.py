...
ax1.set_title("Relation of resistance and temperature for superconductor")
...
for l in [1, 2, 3]:
    for file in files:
        for s in super_select:
            ...
            if file == "T{}V{}.dat".format(np.str_(s),l):
                ...
                print(I)
                print(R)"""
                #plt.plot(I[1:], R[:], marker="x", label="T = {}K".format(s))
    ...
    hall_slope_error = []
    hall_interc_error = []
    for i in range(0, len(super_select)):
        hallx.append(hall_test[i][0])
        hally.append(hall_test[i][2])
        hall_slope_error.append(np.abs(hall_test[i][2]))
        hall_interc_error.append(np.abs(hall_test[i][3]))
    ...
    ax1.errorbar(hallx, hally, yerr = hall_interc_error, label = "experiment set: {}".format(l), capsize = 2)
    hall_test = []
    Rresult = [1, 2, 3, 4]
