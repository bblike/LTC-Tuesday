...
def collect_data(n_pts, next_temp, iinit, step):
    ...
    t.tset = next_temp
    print('Temperature setting from {} {} to {}'.format(t.temp[0], t.temp[1], next_temp))
    ...
    while True:
        ...
        # loop until stable
        if (temp - next_temp) < 0.2 and (next_temp - temp) < 0.2:
            flag += 1
            print('Calibrating...-{}'.format(flag))
        else:
            flag = 0
        sleep(1)
        if flag >= 10:
    ...
    psu.set_voltage = 5
    #psu.set_current = 0.05  # A : write the PSU current setpoint (float, in Ampere units)
    # print('PSU voltage output set to {} V'.format(psu.set_voltage))
    print('PSU voltage output set to {} A'.format(
        psu.set_voltage))  # read back PSU current setpoint value and print to terminal
    print('######################################################################')
    ...
    for p in range(n_pts):
        psu.set_current = iinit + step*p #A - current ramp
        #	or
...
if __name__ == '__main__':
    ...
    object_temp = [80, 90, 100, 103, 105, 106, 107, 108, 109, 110, 115, 120, 130, 140, 150, 160, 170, 180, 190, 200]
    # this line is for a fast testing
    # object_temp = [200]
    # data collection
    for t in object_temp:
        for i in range(1, 4):
            T, V, I, R = collect_data(n_pts, t, Iinit, Istep)
            if len(R) == n_pts:
